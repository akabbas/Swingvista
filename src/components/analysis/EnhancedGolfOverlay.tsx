...
export interface EnhancedGolfOverlayProps {
...
export default function EnhancedGolfOverlay({
  videoRef,
  canvasRef,
  poses,
  phases = [],
  ...
  const render = useCallback(() => {
    ...
    if (showComparison && comparisonPoses.length > 0) {
      drawComparisonView(ctx, landmarks, comparisonPoses, currentTime, canvas.width, canvas.height);
    }

  }, [
    currentTime, poses, phases, clubPathHistory, angleHistory,
    showClubPath, showAngleMeasurements, showPhaseIndicators, showComparison, comparisonPoses
  ]);
