  ...
  public async testConnection(): Promise<{ success: boolean; error?: string; details?: unknown; }> {
    try {
      ...
      const supabaseResponse = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('Supabase connection timeout')), 5000))
      ]) as Response;
      if (!supabaseResponse.ok) {
        return { success: false, error: `Supabase connection failed: ${supabaseResponse.status} ${supabaseResponse.statusText}` };
      }
      let apiSuccess = false;
      try {
        const apiResponse = await Promise.race([
          fetch(`${this.config.apiBaseUrl}/test-supabase`),
          new Promise((_, reject) => setTimeout(() => reject(new Error('API connection timeout')), 3000))
        ]) as Response;
        apiSuccess = apiResponse.ok;
      } catch {}
      return { success: true, details: { supabase: supabaseResponse.ok, api: apiSuccess, environment: this.config.isDevelopment ? 'development' : 'production', timestamp: new Date().toISOString() } };
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : 'Unknown connection error' };
    }
  }

  public getEnvironmentBanner(): string | null { return this.config.isDevelopment ? 'Development Environment - Debug mode enabled' : null; }
