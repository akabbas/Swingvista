...
export interface PhaseDetectionConfig {
  minPhaseDuration: number; // minimum frames for a phase
  velocityThreshold: number; // threshold for detecting movement
  accelerationThreshold: number; // threshold for detecting impact
...
export class SwingPhaseDetector {
  ...
  /**
   * Detect all swing phases from trajectory data
   */
  detectPhases(
  ...
  private detectSetupEnd(landmarks: PoseLandmark[][], trajectory: SwingTrajectory): number {
    ...
    for (let i = 1; i < Math.min(30, rightWrist.length); i++) {
      ...
      if (velocity > this.config.velocityThreshold) {
        return Math.max(this.config.minPhaseDuration, i - 1);
      }
    }
  ...
  /**
   * Detect top of backswing (when club changes direction)
   */
  private detectBackswingTop(landmarks: PoseLandmark[][], trajectory: SwingTrajectory): number {
    const rightWrist = trajectory.rightWrist;
    
    if (rightWrist.length < 2) return Math.min(15, landmarks.length - 1);
    
    let maxY = rightWrist[0].y;
    let topFrame = 0;

    // Look for the highest point (minimum Y value) in the first 70% of the swing
    const searchEnd = Math.min(Math.floor(rightWrist.length * 0.7), rightWrist.length - 1);
  ...
  /**
   * Detect impact frame (maximum acceleration)
   */
  private detectImpact(landmarks: PoseLandmark[][], trajectory: SwingTrajectory): number {
    ...
    // Look for maximum acceleration in the second half of the swing
    const searchStart = Math.floor(rightWrist.length * 0.5);
  ...
  /**
   * Detect start of follow-through (after impact)
   */
  private detectFollowThroughStart(
    landmarks: PoseLandmark[][],
    trajectory: SwingTrajectory,
    impactFrame: number
  ): number {
  ...
  private createPhase(
    ...
    return {
      ...
      endTime,
      duration,
      keyLandmarks: landmarks[Math.floor((startFrame + endFrame) / 2)] || [],
      color,
      description
    };
  }
  ...
  private calculateTempoRatio(phases: SwingPhase[]): number {
    ...
    if (!backswingPhase || !transitionPhase) return 1.0;
    
    const backswingTime = backswingPhase.duration;
    const downswingTime = transitionPhase.duration;
  ...
  /**
   * Calculate weight transfer from back foot to front foot
   */
  private calculateWeightTransfer(
    landmarks: PoseLandmark[][],
    startFrame: number,
    impactFrame: number
  ): number {
  ...
  /**
   * Calculate shoulder and hip rotation
   */
  private calculateRotation(
    ...
    if (startLeftShoulder && startRightShoulder && topLeftShoulder && topRightShoulder) {
      const startAngle = Math.atan2(
        startRightShoulder.y - startLeftShoulder.y,
    ...
    if (startLeftHip && startRightHip && topLeftHip && topRightHip) {
      const startAngle = Math.atan2(
        startRightHip.y - startLeftHip.y,
  ...
  /**
   * Get phase progress (0-1) at a specific frame
   */
  ...
  /**
   * Smooth phase boundaries to avoid jitter
   */
  smoothPhaseBoundaries(phases: SwingPhase[]): SwingPhase[] {
    ...
    for (let i = 0; i < smoothed.length; i++) {
      ...
      if (phase.endFrame - phase.startFrame < this.config.minPhaseDuration) {
        phase.endFrame = Math.min(
          phase.startFrame + this.config.minPhaseDuration,
          maxFrame
        );
      }

      // Ensure phases don't overlap with previous phase
      if (i > 0) {
      ...
      // Ensure phase doesn't exceed maximum frame
