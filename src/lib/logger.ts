...
class Logger {
  ...
  private async sendToMonitoring(entry: LogEntry): Promise<void> {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('swingvista_errors', JSON.stringify(errorLogs.slice(-50))); // Keep last 50 errors
      }
    } catch (error) {
  ...
  fatal(message: string, data?: any, context?: string): void {
    if (this.shouldLog(LogLevel.FATAL)) {
      this.addLog(this.createLogEntry(LogLevel.FATAL, message, data, context));
    }
  }
